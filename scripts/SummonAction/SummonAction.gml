...
function SummonAction(_target_x, _target_y, _figure_sprite, _behaviour) : Action() constructor{
	...
	draw = function() {
		if target_x != undefined {
			draw_sprite_ext(figure_sprite, (global.turn_owner == O_Server.enemy), Game.field.get_cell_xy(Game.field.get_cell(target_x, target_y))[0],
	...
	back = function() {
		if target_x != undefined {
			...
			O_BoardDraw.clear();
			O_BoardDraw.set_button_overlay(Behaviours.get_sprite(summon_figure), (global.turn_owner==O_Server.enemy))
