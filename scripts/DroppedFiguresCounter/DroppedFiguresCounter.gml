...
function DroppedFiguresCounter(_owner, _position = 1) constructor{
	figures = [];
	owner = _owner;
	position = _position
	...
	sort = function(_use_animation = true) {
		...
		for (i = 0; i < array_length(figures); i++) {
			...
			new_figure_x = O_BoardDraw.drop_cord[0] + 30*_scale + figure_x_offset;
			new_figure_y = O_BoardDraw.drop_cord[1] + 7*_scale + 55*_scale*facing + figure_y_offset*facing;
